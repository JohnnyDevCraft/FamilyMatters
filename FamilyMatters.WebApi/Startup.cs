...
using FamilyMatters.WebApi.JWT;
using DevExpress.ExpressApp.Security.Authentication;
using DevExpress.ExpressApp.Security.Authentication.ClientServer;
using FamilyMatters.WebApi.Core;
using DevExpress.ExpressApp.AspNetCore.WebApi;

namespace FamilyMatters.WebApi;
...
public class Startup {
    ...
    public void ConfigureServices(IServiceCollection services) {
        ...
        services.AddXafAspNetCoreSecurity(Configuration, options => {
            ...
            options.UserType = typeof(FamilyMatters.Module.BusinessObjects.ApplicationUser);
            // ApplicationUserLoginInfo is only necessary for applications that use the ApplicationUser user type.
            // If you use PermissionPolicyUser or a custom user type, comment out the following line:
            options.UserLoginInfoType = typeof(FamilyMatters.Module.BusinessObjects.ApplicationUserLoginInfo);
        ...
        services.AddSwaggerGen(c => {
            ...
            c.SwaggerDoc("v1", new OpenApiInfo {
                Title = "FamilyMatters API",
                Version = "v1",
                Description = @"Use AddXafWebApi(options) in the FamilyMatters.WebApi\Startup.cs file to make Business Objects available in the Web API."
            });
    ...
    public void Configure(IApplicationBuilder app, IWebHostEnvironment env) {
        if(env.IsDevelopment()) {
            ...
            app.UseSwagger();
            app.UseSwaggerUI(c => {
                c.SwaggerEndpoint("/swagger/v1/swagger.json", "FamilyMatters WebApi v1");
            });
        }
        else {
